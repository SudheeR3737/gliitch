...
class Glitch extends EventEmitter {
    constructor(client, nodes, options = {}) {
        super();
        if (!client) throw new Error("[Glitch Error] You didn't provide a valid client");
        if (!nodes) throw new Error("[Glitch Error] You did't provide a lavalink node");
    ...
    removeNode(identifier) {
        if (!identifier) throw new Error(`[Glitch Error] Provide identifier as a parameter of removeNode`)
    ...
    createConnection(options) {
        ...
        if (this.leastUsedNodes.length === 0) throw new Error("[Glitch Error] No nodes are avaliable");
        const node = this.nodes.get(this.leastUsedNodes[0].name || this.leastUsedNodes[0].host);
        if (!node) throw new Error("[Glitch Error] No nodes are avalible");
    ...
    checkConnection(options) {
        ...
        if (!guildId) throw new Error(`[Glitch Connection] you have to Provide guildId`)
        if (!voiceChannel) throw new Error(`[Glitch Connection] you have to  Provide voiceChannel`)
        if (!textChannel) throw new Error(`[Glitch Connection] you have to  Provide texteChannel`);
        //  if(shardId == null) throw new Error(`[Glitch Connection] You must have to Provide shardId`);


        if (typeof guildId !== "string") throw new Error(`[Glitch Connection] guildId must be provided as a string`);
        if (typeof voiceChannel !== "string") throw new Error(`[Glitch Connection] voiceChannel must be provided as a string`);
        if (typeof textChannel !== "string") throw new Error(`[Glitch Connection] textChannel must be provided as a string`);
        //   if(typeof shardId !=="number") throw new Error(`[Glitch Connection] shardId must be provided as a number`);
    ...
    async fetchURL(node, track, source) {
        ...
        if (this.spotify.check(track)) {
            ...
            if (!result) throw new Error("[Glitch Error] No tracks found.");
    ...
    #fetch(node, endpoint, param) {
        return fetch(`http${node.secure ? "s" : ""}://${node.host}:${node.port}/${endpoint}?${param}`, {
            ...
            .then((r) => r.json())
            .catch((e) => {
                throw new Error(`[Glitch Error] Failed to fetch from the lavalink.\n  error: ${e}`);
            });
    }
